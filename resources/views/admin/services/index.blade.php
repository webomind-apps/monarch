...
@section('page-contents')
    <div>
        <a class="btn btn-danger btn-sm" href="{{ route('admin.services.create') }}">
            Add
        </a>
    </div>
            ...
            <tbody>
                @foreach ($services as $index => $service)
                    <tr>
                        ...
                        <td><img src="{{ asset("storage/$service->icon_path") }}" height="50px"></td>
                        <td>{{ $service->name }}</td>
                        <td>
                            ...
                            <form method="POST" id="form-submit-{{ $service->id }}"
                                action="{{ route('admin.services.destroy', $service->id) }}" hidden>
                                @method('DELETE')
                                @csrf
